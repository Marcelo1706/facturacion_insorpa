...
async def authenticate_user(nit: str, password: str, db: AsyncSession) -> dict[str, Any] | Literal[False]:
    db_user = await crud_users.get(db=db, username=nit, is_deleted=False)
...
async def blacklist_token(token: str, db: AsyncSession) -> None:
    ...
    if not await crud_token_blacklist.exists(db, token=token):
